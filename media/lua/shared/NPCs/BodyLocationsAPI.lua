...
-- Based on Konijima's Community API
...
function BodyLocationsAPI.checkHiddenModels(loc)

    local item = group:getLocation(loc)
    local hideModels = customGetVal(item, 4)

    for i = 0, hideModels:size() do
        local h = hideModels.get(i)
        print(h)
    end
